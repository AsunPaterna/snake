...
// <editor-fold desc="Extracciones de método">> 
    private void changePositionColor(Graphics2D g2d, int pl, Rectangle cell, int cellWidth, int cellHeight) {
        ...
        for (Rectangle cell : cells) {
            ...
            for (int pl = 0; pl < bs.maxPlayers; pl++) {
                if (bs.players.get(pl).getPosition() == cellnos[i]) {
                    changePositionColor(g2d, pl, cell, cellWidth, cellHeight);
                }
            }
